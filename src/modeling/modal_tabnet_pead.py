...
def run_pead_pipeline():
    ...
    # Collection buckets (mirrors notebook naming)
    ...
    results_df.write_parquet("/out/results.parquet")
    summary_df.write_parquet("/out/per_fold_summary.parquet")

    with open("/out/histories_clf.json", "w") as f:
        json.dump(clf_histories, f, indent=2)
    with open("/out/histories_reg.json", "w") as f:
...
def main():
    ...
    print(f"\nDownload results:")
    print(f"  modal volume get tabnet-pead-output results.parquet .")
    print(f"  modal volume get tabnet-pead-output per_fold_summary.parquet .")
    print(f"  modal volume get tabnet-pead-output histories_clf.json .")
    print(f"  modal volume get tabnet-pead-output histories_reg.json .")
